...
class BebopDrone:
    ...
    ** BebopDrone.process() shall be called on every tick to process pending messages
    ...
    def __init__(self, frame_callback=None, skip=10, frequency=100, movement_speed=0.1, turning_speed=0.2, annonymous=False):
        ...
        self.attitude_sub   = rospy.Subscriber("/bebop/states/ardrone3/PilotingState/AttitudeChanged", Ardrone3PilotingStateAttitudeChanged, self.onAttitudeChanged, queue_size=1)
        self.orientation_sub= rospy.Subscriber("/bebop/states/ardrone3/CameraState/Orientation", Ardrone3CameraStateOrientation, self.onCameraOrientationChanged, queue_size=1)
        self.battery_sub    = rospy.Subscriber("/bebop/states/common/CommonState/BatteryStateChanged", CommonCommonStateBatteryStateChanged, self.onBatteryLevelChanged, queue_size=1)
        self.status_sub     = rospy.Subscriber("/bebop/states/ardrone3/PilotingState/FlyingStateChanged", Ardrone3PilotingStateFlyingStateChanged, self.onStateChanged, queue_size=1)
        self.image_sub      = rospy.Subscriber("/bebop/image_raw", Image, self.onFrameReceived, queue_size=1)
        # Common setup
        self.cam_cmd        = Twist()
        self.navi_cmd       = Twist()
